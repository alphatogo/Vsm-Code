...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            # conditional_sampling(self.args, self.model, self.val_loader, self.step, num_samples=100,mode="outdistro")
            ...
            if self.step % self.save_interval == 0 and self.step > 0:
                memory_key = self.model.memory.memory_key
                memory_labels = self.model.memory.memory_labels
                queue_key = self.model.memory.queue_key
                queue_total = self.model.memory.queue_total
                memory = {'memory_key': memory_key, 'memory_labels': memory_labels, 'queue_key': queue_key,
                          'queue_total': queue_total}
                f_save = open('./memory.pkl', 'wb')
                pickle.dump(memory, f_save)
                f_save.close()
                self.save()
                self.model.eval()
               
                conditional_sampling(self.args, self.model, self.data, self.step, num_samples=1000, mode="indistro")
                conditional_sampling(self.args, self.model, self.val_loader, self.step, num_samples=1000,mode="outdistro")
        ...
        conditional_sampling(self.args, self.model, self.data, self.step, num_samples=100, mode="indistro")
        conditional_sampling(self.args, self.model, self.val_loader, self.step, num_samples=100, mode="outdistro")
